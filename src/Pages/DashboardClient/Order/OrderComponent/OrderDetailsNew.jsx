        ...
        try {
            ...
            if (order.type === 'delivery') {
                const deliveryData = {
                    "order_id": order.id,
                    "status": "delivery",
                    "delivery_rider_id": delivery.id,
                    "note": ` delivery ${delivery.name} take order ${order.id}`
                }
                /* after select delivery add it to the order throw request */
                const setDelivery = await axios.post(`${orderEndPoint.BASE_URL}${orderEndPoint.admin.action.chooseDelivery}`, {
                    data: deliveryData,
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('clientToken')}`
            ...
            <CardContent>
                <Box sx={{ padding: '5px 10px' }} ref={printRef}>
                    ...
                    */}
                    ...
                    {order.type === 'delivery' && loginUser.user.role === 'admin' &&
                        (
                            <>
