...
export const Row2 = () => {
  ...
  const updateClientStatus = async (clientId, currentStatus) => {
    ...
    try {
      const response = await fetch(`https://highleveltecknology.com/Qtap/api/qtap_clients/${clientId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
                ...
                >
                  ...
                  >
                    <span style={{
                      ...
                      color: row.status === 'active' ? "#ef7d00" : row.status === 'inactive' ? "gray" : "defaultColor",
